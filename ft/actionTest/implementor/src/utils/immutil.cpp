	...
	const SaImmAttrValuesT_2 **attrValues)
{
	...
	memset(&operation->objectName, 0, sizeof(SaNameT));
	return operation;
}
...
CcbUtilOperationData_t *ccbutil_getCcbOpDataByDN(SaImmOiCcbIdT ccbId, const SaNameT *dn)
{
        ...
        while (opData != NULL) {
                if ((saNameLen(dn) == saNameLen(&opData->objectName)) &&
                    (memcmp(saNameGet(dn), saNameGet(&opData->objectName), saNameLen(dn)) == 0))
                        break;
...
char const *immutil_getClassName(struct CcbUtilCcbData *ccb, SaImmHandleT immHandle, const SaNameT *objectName)
{
	...
	SaInt8T tmp[sizeof(SA_IMM_ATTR_CLASS_NAME)];
	strcpy(tmp, SA_IMM_ATTR_CLASS_NAME);
	SaImmAttrNameT classNameAttr[] = { tmp, NULL };
...
{
	static char buffer[MAX_DN_LENGTH] = {0};
	unsigned int klen;
	char *cp;

	//assert(name->length <= SA_MAX_NAME_LENGTH);
	memcpy(buffer, saNameGet(name), saNameLen(name));
...
SaImmClassNameT immutil_get_className(const SaNameT *objectName)
{
	...
	SaInt8T tmp[sizeof(SA_IMM_ATTR_CLASS_NAME)];
	strcpy(tmp, SA_IMM_ATTR_CLASS_NAME);
	SaImmAttrNameT attributeNames[2] = { tmp, NULL };
...
static void copySaImmAttrValuesT(struct Chunk *clist, SaImmAttrValuesT_2 *copy, const SaImmAttrValuesT_2 *original)
{
	...
	if (valueCount == 0)
		return;		/* (just in case...) */
	copy->attrValues = (SaImmAttrValueT*)clistMalloc(clist, valueCount * sizeof(SaImmAttrValueT));
...
SaAisErrorT immutil_saImmOmInitialize(SaImmHandleT *immHandle, const SaImmCallbacksT *immCallbacks, const SaVersionT *version)
{
    /* Version parameter is in/out i.e. must be mutable and should not be
       re-used from previous call in a retry loop. */
    SaVersionT localVer = *version;
	SaVersionT ImmVer={'A',2,11};

	SaAisErrorT rc = saImmOmInitialize(immHandle, immCallbacks, &ImmVer);
	if(rc == SA_AIS_ERR_VERSION)
        {
                immutilError("immutil_saImmOmInitialize rc = %d,ImmVer .minorVersion = %d\n", (int)rc, ImmVer.minorVersion);
                rc = saImmOmInitialize(immHandle, immCallbacks, &localVer);
		immutilError("immutil_saImmOmInitialize rc = %d, localVer.minorVersion = %d\n", (int)rc, localVer.minorVersion);
        }
