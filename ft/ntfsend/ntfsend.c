...
SaNtfNotificationHandleT notificationHandleGlobal;
SaNtfElementIdT additionalInfoId[ADDITIONALINFO_MAX];
			     ...
			     saNotificationParamsT *notificationParams, SaUint16T lengthAdditionalText)
{
	...
	*notificationHeader->notifyingObject = notificationParams->notifyingObject;
	...
	notificationHeader->numAdditionalInfo = notificationParams->numAdditionalInfo;
	char * destPtr = NULL;
	int i =0;
	for(i =0; i< notificationParams->numAdditionalInfo; i++)
	{
		notificationHeader->additionalInfo[i].infoId = additionalInfoId[i];
		notificationHeader->additionalInfo[i].infoType = SA_NTF_VALUE_STRING;

		SaAisErrorT ret = saNtfPtrValAllocate(
					notificationHandleGlobal,
					strlen(notificationParams->additionalInfo[i]) +1,
					(void**) &destPtr,
					&(notificationHeader->additionalInfo[i].infoValue));
			if (ret != SA_AIS_OK) {
				printf("saNtfPtrValAllocate failed");
			}
			strncpy(destPtr, notificationParams->additionalInfo[i],strlen(notificationParams->additionalInfo[i]) +1);
	}
}
				...
				saNotificationParamsT *notificationParams)
{
	...
	notificationParams->numAdditionalInfo =0;
	notificationParams->additionalInfo = NULL;
}
	...
	case SA_NTF_TYPE_ALARM:
		...
		do {
			...
			if (SA_AIS_OK != errorCode && SA_AIS_ERR_TRY_AGAIN != errorCode) {
				...
				notificationHandleGlobal =myAlarmNotification.notificationHandle;
...
int main(int argc, char *argv[])
{
	...
	static struct option long_options[] = {
		{"additionalInfo", required_argument, 0, 'z'},
		{"additionalInfoId", required_argument, 0, 'x'},
	...
	if (argc >= 1) {
		/* Check options */
		while ((current_option = getopt_long(argc, argv, "z:x:a:c:e:E:N:n:p:r:s:b:T:", long_options, NULL)) != -1) {
			optionFlag = SA_TRUE;
			int count = 0;
			int additionalInfo_index = 0;
			...
			case 'z':
				count = 0;
				count = count_characters((char *)optarg,';');
				count++;
				myNotificationAllocationParams.numAdditionalInfo = count;
				myNotificationParams.numAdditionalInfo = count;
				myNotificationParams.additionalInfo = (char **)calloc(count,sizeof(char*));
				add_in =NULL;
				add_in = str_split (optarg,';');
				int i = 0;
				for(i =0 ; i< count; i++) {
					myNotificationParams.additionalInfo[i] = (char*) malloc(strlen(add_in[i])+1);
					strncpy(myNotificationParams.additionalInfo[i], add_in[i],strlen(add_in[i]) );
				}
				break;
			case 'x':
				token_str = strtok((char*)optarg,delim_str);
				while( token_str != NULL )
				{
					additionalInfoId[additionalInfo_index] = atoi(token_str);
					token_str = strtok(NULL, delim_str);
					additionalInfo_index++;
				}
				break;
			case 'a':
	...
	int  i=0;
	for ( i=0;i<myNotificationParams.numAdditionalInfo;i++) free(myNotificationParams.additionalInfo[i]);
	free(myNotificationParams.additionalText);
	free(myNotificationParams.additionalInfo);
	exit(EXIT_SUCCESS);
}
