...
""" This module creates a campaign.xml to expand ntfSubscribeApp.
...
class ntfSubscribeExpandCampaignCreator():
    ...
    def create(self):
        ...
        fd.write(self.camp)
        fd.close()
        return ('SUCCESS', 'ntfSubscribe install campaign created')
...
<upgradeCampaign 
  ...
  safSmfCampaign="safSmfCampaign=ERIC-NtfSubscribeAppExpand">
  <campaignInfo>
    <campaignPeriod/>
  </campaignInfo>"""
...
"""
    ...
    <procWrapupAction>
      <immCCB ccbFlags="0">
        <modify objectDN = "safSg=ntfSubscribeApp,safApp=ntfSubscribeApp" operation = "SA_IMM_ATTR_VALUES_REPLACE">
...
def campaignCreator(fileName, appVer):
    camp  = ntfSubscribeExpandCampaignCreator(fileName, appVer)
    result = ntfSubscribeExpandCampaignCreator.create(camp)
    return result
...
if __name__ == '__main__':
    ...
    camp  = ntfSubscribeExpandCampaignCreator("NtfSubscribeExpandCampaign.xml", options.appVer)
    ntfSubscribeExpandCampaignCreator.create(camp)
