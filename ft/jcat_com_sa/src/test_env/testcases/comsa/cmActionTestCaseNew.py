...
GENERAL INFORMATION:
    ...
    Test cases:
    TC-CM-001 - CM action - calling testString in CLI and check the trace value in log
    TC-CM-002 - CM action - calling testIntParams in CLI and check the trace value in log
    TC-CM-004 - CM action - calling testDerived in CLI and check the trace value in log
    TC-CM-003 - CM action - testing with external regression test script
    TC-FT875-001 - Getting error string from failed admin operation when calling action
    TC-FT872-001 - Testing MR-19180 UC1: Any return type frm Action with IMM OI
    TC-MISC-024 - Error printing from COM SA, with string that contains '@' character
    ...
    Id:
    "CM action - calling testString in CLI and check the trace value in log"
    ==================================
    TEST CASE SPECIFICATION:
    ...
    Id:
    "CM action - calling testIntParams in CLI and check the trace value in log"
    ==================================
    TEST CASE SPECIFICATION:
    ...
    Id:
    "CM action - calling testDerived in CLI and check the trace value in log"
    ==================================
    TEST CASE SPECIFICATION:
    ...
    Id:
    "CM action - testing with external regression test script"
    ==================================
    TEST CASE SPECIFICATION:
    ...
    Id:
    "Getting error string from failed admin operation when calling action"
    ==================================
    Tag:
    TC-FT872-001

    Id:
    "Testing MR-19180 UC1: Any return type frm Action with IMM OI"
    ==================================
    TEST CASE SPECIFICATION:
    ...
    Id:
    "Error printing from COM SA, with string that contains '@' character"
    ==================================
"""
...
class CMaction(coreTestCase.CoreTestCase):
    ...
    def runTest(self):
        ...
        if ComsaOK[1]:
            ...
            self.modelfile_imm  = 'Sdp617ActiontestMom_imm_classes.xml'
            self.modelfile_imm2 = 'Sdp872ActiontestMom_imm_classes.xml'
            self.modelfile_mp   = 'Sdp617ActiontestMom_mp.xml'
            self.modelfile_mp1  = 'Sdp875ActiontestMom_mp.xml'
            self.modelfile_mp2  = 'Sdp872ActiontestMom_mp.xml'
            ...
            if self.testcase_tag == 'TC-FT872-001':
                result = self.sshLib.remoteCopy('%s%s' %(self.modelfile_path,self.modelfile_imm2), self.actionTest_tempdir, timeout = 60)
            else:   # TC-CM-00x and TC-FT875-001
                result = self.sshLib.remoteCopy('%s%s' %(self.modelfile_path,self.modelfile_imm), self.actionTest_tempdir, timeout = 60)
            self.fail(result[0],result[1])

            if self.testcase_tag == 'TC-FT872-001':
                result = self.sshLib.remoteCopy('%s%s' %(self.modelfile_path, self.modelfile_mp2), self.actionTest_tempdir, timeout = 60)
                self.fail(result[0],result[1])
            elif self.testcase_tag == 'TC-FT875-001' or 'TC-MISC-024':
                result = self.sshLib.remoteCopy('%s%s' %(self.modelfile_path, self.modelfile_mp1), self.actionTest_tempdir, timeout = 60)
                self.fail(result[0],result[1])
            else:   # TC-CM-00x
            ...
            if self.testcase_tag == 'TC-FT872-001':
                result = self.comsa_lib.addMomFileToCom(self.activeController[0], self.activeController[1], '%s%s' %(self.actionTest_tempdir, self.modelfile_mp2))
                self.fail(result[0],result[1])
            elif self.testcase_tag == 'TC-FT875-001' or 'TC-MISC-024':
                result = self.comsa_lib.addMomFileToCom(self.activeController[0], self.activeController[1], '%s%s' %(self.actionTest_tempdir, self.modelfile_mp1))
                self.fail(result[0],result[1])
            else:   # TC-CM-00x
            ...
            if self.testcase_tag == 'TC-FT872-001':
                cmd = 'immcfg -f %s%s' %(self.actionTest_tempdir, self.modelfile_imm2)
                result = self.sshLib.sendCommand(cmd, self.activeController[0], self.activeController[1])
            else:   # TC-CM-00x and TC-FT875-001
            ...
            if self.testcase_tag != 'TC-CM-003':
                ...
                for step in range(steps):
                    ...
                    for trace_entry in traces[trace_entry_step: trace_entry_step + traces_checked_this_step]:
                        ...
                        if trace_entry[0] == "[" and trace_entry[-1:] == "]":
                            searchPattern = eval(trace_entry)
                        else:
            ...
            if self.testcase_tag == 'TC-CM-003':
                self.setTestStep('### Testing with external script')
                #Running the regression test script
                self.myLogger.debug('Running the regression test script')

                CmwMajorOK = ('SUCCESS', True)
                ComMajorOK = ('SUCCESS', True)
                ComsaMajorOK = ('SUCCESS', True)
                isRunningComSa3_2Bwd = False

                CmwMajorOK = self.lib.checkComponentMajorVersion('cmw', "1", "6", [], True)
                ComMajorOK = self.lib.checkComponentMajorVersion('com', "2" , "7", [], True)
                ComsaMajorOK = self.lib.checkComponentMajorVersion('comsa', "3", "4", [], True)
                if ComsaMajorOK[1] == False:
                    self.external_script_name = self.external_script_name.replace('runalltests.sh', 'runalltests3.2.sh')
                if CmwMajorOK[1] == False and ComMajorOK[1] == False:
                    isRunningComSa3_2Bwd = True

                SC_ipaddr = self.targetData['ipAddress']['ctrl']['ctrl%s'%str(self.activeController[1])]
                cmd = 'cd %s;bash -c "./%s %s %s"' %(self.external_script_path, self.external_script_name, SC_ipaddr, isRunningComSa3_2Bwd)

                result = self.miscLib.execCommand(cmd)
                self.fail(result[0],result[1])

                if '--- RESULT: All Test PASSED ---' in result[1]:
                    self.logger.info('External regression script result: All test PASSED')
                else:
                    self.fail('ERROR', 'External regression script result: Error/Fail')
        else:
    ...
    def tearDown(self):
        ...
        if self.skip_test == False:
            ...
            #Delete objects
            cmd = 'immcfg -d -u actionTestId=1,sdp617ActiontestRootId=1'
            ...
            cmd = 'immcfg -d -u sdp617ActiontestRootId=1'
            self.myLogger.debug('tearDown: Delete test object: %s' %cmd)
            self.sshLib.sendCommand(cmd, self.activeController[0], self.activeController[1])

            # Below we remove all the classes imported from the imm_classes model file
            if self.testcase_tag == 'TC-FT872-001':
                cmd = """grep "<class" %s/%s | grep name | awk -F'\"' '{ print $2 }' | xargs immcfg --delete-class""" %(self.actionTest_tempdir, self.modelfile_imm2)
            else:   # TC-CM-00x and TC-FT875-001
                cmd = """grep "<class" %s/%s | grep name | awk -F'\"' '{ print $2 }' | xargs immcfg --delete-class""" %(self.actionTest_tempdir, self.modelfile_imm)
            result = self.sshLib.sendCommand(cmd, self.activeController[0], self.activeController[1])
            if result[0] != 'SUCCESS':
            ...
            if self.testcase_tag == 'TC-FT872-001':
                result = self.comsa_lib.removeMomFileFromCom(self.activeController[0], self.activeController[1], '%s%s' %(self.actionTest_tempdir, self.modelfile_mp2))
                self.fail(result[0],result[1])
            elif self.testcase_tag == 'TC-FT875-001' or 'TC-MISC-024':
                result = self.comsa_lib.removeMomFileFromCom(self.activeController[0], self.activeController[1], '%s%s' %(self.actionTest_tempdir, self.modelfile_mp1))
                self.fail(result[0],result[1])
            else:   # TC-CM-00x
                result = self.comsa_lib.removeMomFileFromCom(self.activeController[0], self.activeController[1], '%s%s' %(self.actionTest_tempdir, self.modelfile_mp))
                self.fail(result[0],result[1])
    ...
    def load_expected_trace_lists(self):
        ...
        trace_list = []
        dic_trace_list = {}
        # change the way of adding to trace list for improvement task43817, in case number of expected_trace_value > 20
        # index is the number in the attribute (i.e expected_trace_value_10 -> index is 10)
        # Add expected trace value to trace_list in the order from lowest to highest index
        for expected_trace_value in [attr for attr in dir(self) if attr.startswith('expected_trace_value')]:
            temp_expected_trace_value = eval('self.' + expected_trace_value)
            if temp_expected_trace_value != {}:
                try:
                    index = int(expected_trace_value[expected_trace_value.rfind('_')+1:])
                    dic_trace_list[index] = temp_expected_trace_value
                except:
                    self.myLogger.error('Failed to get index number for %s' % expected_trace_value)

        for key in sorted(dic_trace_list):
            trace_list.append(dic_trace_list[key])
