...
void Action_Storage::add(std::string ACTION, ComOamSpiMoAttributeType_3T ACTION_TYPE)
{
	actionTest_Map.insert(std::pair<std::string, ComOamSpiMoAttributeType_3T>(ACTION, ACTION_TYPE));
}
 ...
 *
 */
ComOamSpiMoAttributeType_3T Action_Storage::get(const char* ActionTestDN, const char* methodName)
{
	//syslog(LOG_INFO, "%s ENTER function getActionParamType",log_prefix);
	std::string actionTestMethodPath = ActionTestDN;
	ComOamSpiMoAttributeType_3T returnActionParamType;
	...
	{
//		returnActionParamType = ComOamSpiMoAttributeType_VOID;
	    returnActionParamType = ComOamSpiMoAttributeType_3_INT8;  // used to be _VOID, but this is not supported in MO SPI Ver.3
