 ...
 * From these outputs: It generates the corresponding: -ComReturnT, -syslogs, -addMessage (if necessary)
 *
 * Input: parent_Dn, class_Name
 *
 * Returns: permissionRet (type: ComReturnT)
 *
...
ComReturnT Registration_Storage::get(ComMgmtSpiThreadContext_2T* _threadContextIf, const char* parent_Dn, const char* class_Name = 0, const char* key_Attr = 0)
{
	//syslog(LOG_INFO, "%s ENTER function CheckPermission",log_prefix);
	permissionType permission = permissionNotAvailable;
	ComReturnT permissionRet = ComFailure;
	...
	case permissionNO:
		...
		syslog(LOG_INFO, "%s function CheckPermission: sending ComNotExist",log_prefix);
		permissionRet = ComNotExist;
		break;
	case permissionNotAvailable:
		...
		nbi_message += component_name;
		_threadContextIf->addMessage(ThreadContextMsgNbi_2, nbi_message.c_str());
		syslog(LOG_INFO, "%s function CheckPermission: sending ComFailure",log_prefix);
		permissionRet = ComFailure;
		break;
	}
...
ComReturnT Registration_Storage::maf_get(MafMgmtSpiThreadContext_2T* _threadContextIf, const char* parent_Dn, const char* class_Name = 0, const char* key_Attr = 0)
{
	//syslog(LOG_INFO, "%s ENTER function CheckPermission",log_prefix);
	permissionType permission = permissionNotAvailable;
	ComReturnT permissionRet = ComFailure;
...
ComReturnT Registration_Storage::get_ReturnValue(std::string functionName)
{
