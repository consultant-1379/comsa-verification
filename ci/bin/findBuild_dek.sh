...
set NEIGHBORS_FOLDER="/home/comsaci/neighbors/comsa/"
...
if ( $BUILD_TYPE == "CP" ) then
  ...
  set FOLDERS=`ls -d "${NEIGHBORS_FOLDER}${MAJOR_VERSION}_CP${RELEASE_VERSION}"`
  ...
  set FOLDERS=`ls --color=never -d ${RELEASE_FOLDER}${COMSA_BRANCH}-${MAJOR_VERSION}_SH${RELEASE_VERSION}-${BUILD_TYPE}${BUILD_DAY}.*`
else
