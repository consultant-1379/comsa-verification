...
if ( "$7" == '-f' ) then
  ...
  set COMSA_TARGET_DIR=`findBuild.sh ${COMSA_BRANCH} SN`
endif
if ( "$7" == '-a' ) then
...
setenv COMSA_VERIFICATION_COMMIT_ID "`git --git-dir ${COMSA_TEST_REPO}/.git rev-parse --short --default master`"
setenv COMSA_REPO_PATH_COMMIT_ID "`git --git-dir ${COMSA_SOURCE_REPO}/.git rev-parse --short HEAD`"
...
foreach TEST_SUITE ($TEST_SUITES:q)
  ...
  ciTest.sh ${RUN_TEST_DIR} ${COMSA_TARGET_DIR} $TEST_SUITE:q ${ADDITIONAL_CMD} ${ADDITIONAL_PARAM}
  if ( $status != 0 ) then
